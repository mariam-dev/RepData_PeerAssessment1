...
```{r meanSteps, results='asis', fig.height=4}
library(ggplot2)
...
<span style="color: blue;"> Observations: The mean for the total number of steps is `r stepMean` and  the median is `r stepMedian`. Since the mean and median are close, we can expect a symmetrical distribution</span>.
...
print(maxInterval)
```
<span style="color: blue;"> Observations: The maximum number of steps is `r maxInterval$steps` and  can be found in interval with identifier `r maxInterval$interval`</span>.
...
```
<span style="color: blue;"> Observations: Only steps has missing values that account for `r percentage`%. This might definately introduce a bias and a strategy should be  considered for these values.
</span>.
...
nadata <- activityData %>% filter(is.na(steps)) %>% group_by(dateformat)%>%summarize(affected = TRUE)
sumnas <- sum(daysteps2$steps-daysteps$daysteps)
perc <- sumnas/sum(activityData$steps, na.rm=TRUE)*100

affected <- nadata$dateformat

```

<span style="color: blue;"> Observations: The mean for the total number of steps is `r stepMean2` and  the median is `r stepMedian2`. The mean remains the same while the median slightly increased since the average steps is in the substituted values is always greater than zero. The sum of the steps that are included in the strategy represent only `r perc` of the total steps taken without considering the NAs.
The affected dates are: `r affected` 
</span>.
